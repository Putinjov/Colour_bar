...
app.post("/api/bookings", async (req, res) => {
    ...
    if (!start.isValid)
        return res.status(400).send("Invalid startAt");
    ...
    if (overlap)
        return res.status(409).send("Цей час уже зайнятий");
...
app.post("/api/admin/block", async (req, res) => {
    ...
    if (overlap)
        return res.status(409).send("Range overlaps existing booking/block");
    const created = await Booking.create({
        kind: "blocked",
        startAt: start.toJSDate(),
        endAt: end.toJSDate(),
